            ...
            if (data.success) {
                ...
                if (data.listModel.length > 0) {
                    ...
                    for (var i = data.listModel.length - 1; i >= 0 ; i--) {
                        table += "<td style='border: none;height: 17px; width: 20px; background-color:" + data.listModel[i].NewStatusColorHex + ";'></td>";
...
function SetUpProgressBar(callslip, id) {
    //$('#divProgressBar').hide();
    $('#' + id).parent().find('#divProgressBar').show();
    $('#' + id).parent().find('#divProgressBar').html("");
    $('#' + id).parent().find('#divProgressBar').load("Home/ProgressBar", function () {
        if (callslip != null) {

            ////Step1: Make Progress Steps Green or Red.
            //if (callslip.IsAccepted) { $('.accepted').addClass('green'); } else { $('.accepted').addClass('red'); }
            //if (callslip.IsAssigned) { $('.assigned').addClass('green'); } else { $('.assigned').addClass('red'); }
            //if (callslip.IsDispatched) { $('.dispatched').addClass('green'); } else { $('.dispatched').addClass('red'); }
            //if (callslip.IsReturn) { $('.return').addClass('green'); } else { $('.return').addClass('red'); }
            //if (callslip.IsPartsOnOrder) { $('.partsonorder').addClass('green'); } else { $('.partsonorder').addClass('red'); }
            //if (callslip.IsFollowUp) { $('.followup').addClass('green'); } else { $('.followup').addClass('red'); }
            //if (callslip.IsPictures) { $('.pictures').addClass('green'); } else { $('.pictures').addClass('red'); }
            //if (callslip.IsComplete) { $('.complete').addClass('green'); } else { $('.complete').addClass('red'); }
            
            //Step2: check if updates are required. If (YES){check if its portal(P) or email(E) updates and display it.}else than display P on required updates else display E
            if (callslip.RequireUpdates) {
                //Created
                if (callslip.UpdateOnCreated) {
                    if (callslip.SendUpdateInCustomerPortal) {
                        if (callslip.UpdatedOnAcceptedInPortal) {
                            $('.accepted').removeClass('visited');
                            $('.accepted').removeClass('disabled');
                            $('.accepted').addClass('active');
                        } else {
                            $('.accepted').removeClass('active');
                            $('.accepted').removeClass('disabled');
                            $('.accepted').addClass('visited');
                        }
                    } else {
                        $('.accepted').removeClass('active');
                        $('.accepted').removeClass('visited');
                        $('.accepted').addClass('disabled');
                    }
                    if (callslip.SendUpdateInCustomerEmail) {
                        if (callslip.UpdatedOnAcceptedInEmail) {
                            $('.accepted').removeClass('visited');
                            $('.accepted').removeClass('disabled');
                            $('.accepted').addClass('active');
                        } else {
                        ...
                        //$('.accepted').find('.ringButtonEmail').hide();
                    }
                }
                else {
                    $('.accepted').removeClass('active');
                ...
                if (callslip.UpdateOnDispatched) {
                    if (callslip.SendUpdateInCustomerPortal) {
                        if (callslip.UpdatedOnDispatchInPortal) {
                            $('.dispatched').removeClass('visited');
                            $('.dispatched').removeClass('disabled');
                            $('.dispatched').addClass('active');
                        } else {
                            $('.dispatched').removeClass('active');
                            $('.dispatched').removeClass('disabled');
                            $('.dispatched').addClass('visited');
                        }
                    } else {
                        $('.dispatched').removeClass('active');
                        $('.dispatched').removeClass('visited');
                        $('.dispatched').addClass('disabled');
                    }
                    if (callslip.SendUpdateInCustomerEmail) {
                        if (callslip.UpdatedOnDispatchInEmail) {
                            $('.dispatched').removeClass('visited');
                            $('.dispatched').removeClass('disabled');
                            $('.dispatched').addClass('active');
                        } else {
                        ...
                        //$('.dispatched').find('.ringButtonEmail').hide();
                    }
                }
                else {
                    $('.dispatched').removeClass('active');
                ...
                if (callslip.UpdateOnDelayed) {
                    if (callslip.SendUpdateInCustomerPortal) {
                        if (callslip.UpdatedOnDelayedInPortal) {
                            $('.delayed').removeClass('visited');
                            $('.delayed').removeClass('disabled');
                            $('.delayed').addClass('active');
                        } else {
                            $('.delayed').removeClass('active');
                            $('.delayed').removeClass('disabled');
                            $('.delayed').addClass('visited');
                        }
                    } else {
                        $('.delayed').removeClass('active');
                        $('.delayed').removeClass('visited');
                        $('.delayed').addClass('disabled');
                    }
                    if (callslip.SendUpdateInCustomerEmail) {
                        if (callslip.UpdatedOnDelayedInEmail) {
                            $('.delayed').removeClass('visited');
                            $('.delayed').removeClass('disabled');
                            $('.delayed').addClass('active');
                        } else {
                        ...
                        //$('.delayed').find('.ringButtonEmail').hide();
                    }
                }
                else {
                    $('.delayed').removeClass('active');
                ...
                if (callslip.UpdateOnCompleted) {
                    if (callslip.SendUpdateInCustomerPortal) {
                        if (callslip.UpdatedOnCompleteInPortal) {
                    ...
                    if (callslip.SendUpdateInCustomerEmail) {
                        if (callslip.UpdatedOnCompleteInEmail) {
                            $('.complete').removeClass('visited');
                            $('.complete').removeClass('disabled');
                            $('.complete').addClass('active');
                        } else {
                        ...
                        //$('.complete').find('.ringButtonEmail').hide();
                    }
                }
                else {
                    $('.complete').removeClass('active');
