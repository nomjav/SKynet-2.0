...
function OpenJob(id) {
    ...
    $('<div id="'+id+'"></div>').dialog({
        ...
        modal: false,
        open: function (e) {
            $(e.target).parent().css('background-color', '#eeeeee');
...
function showEstimatesDiv(id) {
    ...
    GetPartsDetailsByCallSlipId(id);
    //-----------------------------
...
function PopulateDocuments(documents, id) {
    ...
    documentslist += "</select> </th><th></th><th><button id='RefreshDocuments' style='float: right;' onclick='GetDocumentsByCallSlipId();return false;' class='btn btn-primary'><i class='fa fa-refresh'></i></button></th></tr></thead><tbody class='tableBody'>";
    if (documents.length > 0) {
        ...
        for (var i = 0; i < documents.length; i++) {
            ...
            documentslist += "<tr><td><a style='width:200px;color:#000;' target='_blank' href='" + filepath + "'>" + documents[i].Title + "</a>" +
                "</td><td><b>" + documents[i].UserName + " - " + documents[i].CreatedOn + "</b></td><td><button onclick='DeleteDocumentByCallSlipIdDocumentId(" + documents[i].Id + ");return false;' class='btn btn-danger'><i class='fa fa-trash'></i></button></td>";
...
function GetTablesDataByJobId(id) {
    ...
    var storeNo = $('#ServiceTab_StoreNumber1').val();
...
function GetEmailsLogsByCustomerId() {
    ...
    if (customerId != null) {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json, charset=utf-8",
            url: "Customer/GetEmailsLogsByCustomerId",
            data: JSON.stringify(request),
            global: false,
            cache: false,
            success: function (msg) {
...
function UpdateJob() {
    ...
    var allowComm = false;
    var request = {
        CallSlipId: $('#CallSlipId').val(), CustomerType: $('#CustomerType').val(), JobTypeId: $('#JobTypeId').val(),
...
function GetPartsDetailsByCallSlipId() {
    var request = { CallSlipId: $('#CallSlipId').val() };
    $.ajax({
        type: "POST",
        contentType: "application/json, charset=utf-8",
        url:"Home/GetPartsDetailsByCallSlipId",
        data: JSON.stringify(request),
        global: false,
        cache: false,
        success: function (msg) {
            ...
            if (data.success) {
                ...
                for (var i = 0; i < data.parts.length; i++) {
                    estimateBody += "<tr>";
                    estimateBody += "<td>" + data.parts[i].EstimateNumber + "</td>";
                    estimateBody += "<td>" + data.parts[i].EstimateLabel + "</td>";
                    estimateBody += "<td>" + data.parts[i].Total + "</td>";
                    estimateBody += "<td>" + data.parts[i].CreatedOn + "</td>";
                    estimateBody += "<td>" + (data.parts[i].Approved ? 'Yes' : 'No') + "</td>";
                    estimateBody += '<td><button onclick="CreateEditEstimate(' + data.parts[i].EstimateNumber + ', ' + true + ');"  class="btn btn-success EditEstimate"><i class="fa fa-edit"></i></button></td>';
                    estimateBody += "</tr>";
                }
                //
                $('#estimatesBody').html("");
                $('#estimatesBody').html(estimateBody);
...
function GetPartsByCallSlipId(estNum) {
    var request = { CallSlipId: $('#CallSlipId').val(), EstimateNumber: estNum };
    $.ajax({
        type: "POST",
        contentType: "application/json, charset=utf-8",
        ...
        success: function (msg) {
            ...
            if (data.success) {
                if (data.approvedStatus == true) {
                    ...
                    $('#approvedBtn').val("Approve Estimate");
                    $('#approvedBtn').removeAttr("class");
                    $('#approvedBtn').addClass("btn btn-warning");
